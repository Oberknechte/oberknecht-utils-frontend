...
import {
  ...
  tableOptionsDropdownSortOptionEntry,
  tableOptionsFilterEntryInternal,
  tableOptionsType,
...
export class elements {
  ...
  static createTable = (tableOptions: tableOptionsType) => {
    if (!tableOptions) tableOptions = {} as tableOptionsType;
    ...
    let tablezTopContainerElem: HTMLDivElement =
      tableContainer.querySelector(`#${tableID}-ztop-container`) ??
      elements.createElement("div", {
        pe: tableContainer,
        classes: [
    ...
    let tableContainerTop: HTMLDivElement =
    ...
    let tableContainerTop2: HTMLDivElement =
    ...
    let tableContainerBottom: HTMLDivElement =
      (tableContainer.querySelector(
    ...
    let tableDropdownSortContainerElem: HTMLDivElement = !tableOptions.dropdownSort
      ? undefined
    ...
    let defaultSortOption: tableOptionsDropdownSortOptionEntry;
    ...
    let tableSearchInputElem: HTMLInputElement = !tableOptions.search
      ? undefined
      : (tableSearchContainerElem.querySelector(
    ...
    (() => {
      ...
      tableSearchInputElem.oninput = () => {
        ...
        if (!tableOptions.searchOptions?.callback) return search();
    ...
    if (!tableExists) {
      ...
      let tableSortOptions: Record<string, any> = {
      ...
      tableOptions.names.forEach((name: string, i) => {
        let th = elements.createElement("th", {
          ...
          innerText: name,
          onclick: () => {
            if (tableOptions.noSort || name.toString().length === 0) return;

            let lastSortIndex = tableSortOptions.lastSortIndex;
            let lastSortMode = tableSortOptions.sortMode;
            let lastSortMode_ =
              isNullUndefined(lastSortIndex) || lastSortIndex !== i
                ? 0
                : lastSortMode;
    ...
    if (tableOptions.dropdownSort) {
      ...
      let tableSortDropdownSelect = elements.createElement("select", {
        ...
        onchange: () => {
          currentSortOption =
            tableOptions.dropdownSortOptions[
              parseInt(tableSortDropdownSelect.value)
            ];
          changeSort();
        },
      });
  ...
  static sortTable = (options: sortTableOptionsType) => {
    ...
    let sortAttributeNames = convertToArray(options.sortAttributeNames);
    ...
    let trSorted = trs
      .map((a, i) => {
        ...
        let val =
          sortAttributeName && a.childNodes[tdNum].firstChild
            ? // @ts-ignore
  ...
  static hasParentAny = (
    ...
    function checkParent(elem_) {
      if (isNullUndefined(elem_)) return;
      if (elem_.parentNode === parentElem) return (r = true);
